...
function appendMessage(text, sender, isTyping = false, isInitialGreeting = false, isError = false, errorDetails = null, fromHistory = false) {
    ...
    if (sender === 'bot' && !isTyping && !isError && (isInitialGreeting || fromHistory)) { addCopyButton(messageContentWrapper, text, messageBubble); }
...
async function sendMessage() {
    ...
    try {
        ...
        if(chatbox) chatbox.appendChild(botMsgContainer);
